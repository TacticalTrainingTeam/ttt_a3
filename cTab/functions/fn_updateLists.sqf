...
*/
{
	if ((_x != cTab_player) && {[_x,["ACE_DK10_b","ACE_GD300_b","ItemMicroDAGR"]] call cTab_fnc_checkGear}) then {
...
*/
{
	if ((side _x in _validSides) && {_x != _playerGroup}) then {
		_leader = objNull;
		call {
			if ([leader _x,["ACE_DK10_b","ACE_GD300_b"]] call cTab_fnc_checkGear) exitWith {_leader = leader _x;};
			{
				if ([_x,["ACE_DK10_b","ACE_GD300_b"]] call cTab_fnc_checkGear) exitWith {_leader = _x;};
			} count units _x;
		};
...
*/
{
	if ((side _x in _validSides) && {count (crew _x) > 0} && {_x != _playerVehicle} && {{[_x,["ACE_DK10_b","ACE_GD300_b"]] call cTab_fnc_checkGear} count (crew _x) > 0}) then {
...
Units on our side, that have either helmets that have been specified to include a helmet cam, or ACE_HelmetCam in their inventory.
*/
{
	if (side _x in _validSides) then {
		if (headgear _x in cTab_helmetClass_has_HCam || {[_x,["ACE_HelmetCam"]] call cTab_fnc_checkGear}) then {
