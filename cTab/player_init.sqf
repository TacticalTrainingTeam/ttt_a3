...
cTab_fnc_onIfMainPressed = {
	...
	_interfaceName = call {
		...
		if ([_player,["ACE_GD300_b"]] call cTab_fnc_checkGear) exitWith {"cTab_Android_dsp"};
		if ([_player,["ItemMicroDAGR"]] call cTab_fnc_checkGear) exitWith {
			cTabMicroDAGRmode = if ([_player,["ACE_DK10_b"]] call cTab_fnc_checkGear) then {0} else {2};
			"cTab_microDAGR_dsp"
		};
		if ([_player,_vehicle,"FBCB2"] call cTab_fnc_unitInEnabledVehicleSeat) exitWith {"cTab_FBCB2_dlg"};
		if ([_player,["ACE_DK10_b"]] call cTab_fnc_checkGear) exitWith {"cTab_Tablet_dlg"};
		// default
		""
	};
...
cTab_fnc_onIfSecondaryPressed = {
	...
	_interfaceName = call {
		...
		if ([_player,_vehicle,"FBCB2"] call cTab_fnc_unitInEnabledVehicleSeat) exitWith {"cTab_FBCB2_dlg"};
		if ([_player,["ACE_GD300_b"]] call cTab_fnc_checkGear) exitWith {"cTab_Android_dlg"};
		if ([_player,["ItemMicroDAGR"]] call cTab_fnc_checkGear) exitWith {
			cTabMicroDAGRmode = if ([_player,["ACE_DK10_b"]] call cTab_fnc_checkGear) then {0} else {2};
			"cTab_microDAGR_dlg"
		};
...
cTab_msg_gui_load = {
	...
	{
		if ((side _x in _validSides) && {isPlayer _x} && {[_x,["ACE_DK10_b","ACE_GD300_b"]] call cTab_fnc_checkGear}) then {
...
["cTab_msg_receive",
	{
		...
		if (_msgRecipient == cTab_player && _sender != cTab_player && {_playerEncryptionKey == _msgEncryptionKey} && {[cTab_player,["ACE_DK10_b","ACE_GD300_b"]] call cTab_fnc_checkGear}) then {
