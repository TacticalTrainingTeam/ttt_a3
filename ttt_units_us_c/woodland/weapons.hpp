	/*
	#############################################################
	### Uniformen
	#############################################################
	*/
	class TTT_Uniform_Gelb_Us_Woodland : TTT_Uniform_Gelb_Us_Desert {
		displayName = "TTT Uniform Gelb (US Woodland)";
		
		class ItemInfo : TTT_Uniform_ItemInfo {
			uniformClass = "TTT_Gelb1_Us_Woodland";
		};
	};
	
	class TTT_Uniform_Grau_Us_Woodland : TTT_Uniform_Grau_Us_Desert {
		displayName = "TTT Uniform Grau (US Woodland)";
		
		class ItemInfo : TTT_Uniform_ItemInfo {
			uniformClass = "TTT_Grau1_Us_Woodland";
		};
	};
	
	class TTT_Uniform_Gold_Us_Woodland : TTT_Uniform_Gold_Us_Desert {
		displayName = "TTT Uniform Gold (US Woodland)";
		
		class ItemInfo : TTT_Uniform_ItemInfo {
			uniformClass = "TTT_Gold1_Us_Woodland";
		};
	};
	
	class TTT_Uniform_Gruen_Us_Woodland : TTT_Uniform_Gruen_Us_Desert {
		displayName = "TTT Uniform Gruen (US Woodland)";
		
		class ItemInfo : TTT_Uniform_ItemInfo {
			uniformClass = "TTT_Gruen1_Us_Woodland";
		};
	};
	
	class TTT_Uniform_Braun_Us_Woodland : TTT_Uniform_Braun_Us_Desert {
		displayName = "TTT Uniform Braun (US Woodland)";
		
		class ItemInfo : TTT_Uniform_ItemInfo {
			uniformClass = "TTT_Braun1_Us_Woodland";
		};
	};
	
	class TTT_Uniform_Schwarz_Us_Woodland : TTT_Uniform_Schwarz_Us_Desert {
		displayName = "TTT Uniform Schwarz (US Woodland)";
		
		class ItemInfo : TTT_Uniform_ItemInfo {
			uniformClass = "TTT_Schwarz1_Us_Woodland";
		};
	};
	
	class TTT_Uniform_Rot_Us_Woodland : TTT_Uniform_Rot_Us_Desert {
		displayName = "TTT Uniform Rot (US Woodland)";
		
		class ItemInfo : TTT_Uniform_ItemInfo {
			uniformClass = "TTT_Rot1_Us_Woodland";
		};
	};	
	
	class TTT_Uniform_Bronze_Us_Woodland : TTT_Uniform_Bronze_Us_Desert {
		displayName = "TTT Uniform Bronze (US Woodland)";
		
		class ItemInfo : TTT_Uniform_ItemInfo {
			uniformClass = "TTT_Bronze1_Us_Woodland";
		};
	};
	
	class TTT_Uniform_Blau_Us_Woodland : TTT_Uniform_Blau_Us_Desert {
		displayName = "TTT Uniform Blau (US Woodland)";
		
		class ItemInfo : TTT_Uniform_ItemInfo {
			uniformClass = "TTT_Blau1_Us_Woodland";
		};
	};	
	
	class TTT_Uniform_Violett_Us_Woodland : TTT_Uniform_Violett_Us_Desert {
		displayName = "TTT Uniform Violett (US Woodland)";
		
		class ItemInfo : TTT_Uniform_ItemInfo {
			uniformClass = "TTT_Violett1_Us_Woodland";
		};
	};	
	
	class TTT_Uniform_Weiss_Us_Woodland : TTT_Uniform_Weiss_Us_Desert {
		displayName = "TTT Uniform Weiss (US Woodland)";
		
		class ItemInfo : TTT_Uniform_ItemInfo {
			uniformClass = "TTT_Weiss1_Us_Woodland";
		};
	};
	
	class TTT_Uniform_Silber_Us_Woodland : TTT_Uniform_Silber_Us_Desert {
		displayName = "TTT Uniform Silber (US Woodland)";
		
		class ItemInfo : TTT_Uniform_ItemInfo {
			uniformClass = "TTT_Adler1_Us_Woodland";
		};
	};
		
	/*
	#############################################################
	### Helme
	#############################################################
	*/
	class TTT_Helmet_1_Us_Woodland : TTT_Helmet_1_Us_Desert {
		displayName = "TTT Helm I";
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\helmets\TTT_helmet_1_us_woodland.paa"};
	};
	
	class TTT_Helmet_2_Us_Woodland : TTT_Helmet_2_Us_Desert {
		displayName = "TTT Helm II";
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\helmets\TTT_helmet_2_us_woodland.paa"};
	};
	
	class TTT_Helmet_3_Us_Woodland : TTT_Helmet_3_Us_Desert {
		displayName = "TTT Helm III";
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\helmets\TTT_helmet_3_us_woodland.paa"};
	};
	
	class TTT_Helmet_4_Us_Woodland : TTT_Helmet_4_Us_Desert {
		displayName = "TTT Helm IV";
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\helmets\TTT_helmet_4_us_woodland.paa"};
	};
	
	class TTT_Helmet_5_Us_Woodland : TTT_Helmet_5_Us_Desert {
		displayName = "TTT Helm V";
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\helmets\TTT_helmet_5_us_woodland.paa"};
	};
	
	class TTT_Helmet_6_Us_Woodland : TTT_Helmet_6_Us_Desert {
		displayName = "TTT Helm VI";
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\helmets\TTT_helmet_6_us_woodland.paa"};
	};
	
		/*
	#############################################################
	### Westen
	#############################################################
	*/
	
	class TTT_Vest_Heavy_Yellow_Us_Woodland : TTT_Vest_Heavy_Base {
		scope = 2;
		displayName = "TTT Schwere Weste Gelb (US Woodland)";
		hiddenSelections[] = {"camo"};
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\vests\vest_yellow_co.paa"};
	};
	
	class TTT_Vest_Lite_Yellow_Us_Woodland : TTT_Vest_Lite_Base {
		scope = 2;
		displayName = "TTT Leichte Weste Gelb (US Woodland)";
		hiddenSelections[] = {"camo"};
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\vests\vest_yellow_co.paa"};
	};
	
	class TTT_Vest_Heavy_Blue_Us_Woodland : TTT_Vest_Heavy_Base {
		scope = 2;
		displayName = "TTT Schwere Weste Blau (US Woodland)";
		hiddenSelections[] = {"camo"};
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\vests\vest_blue_co.paa"};
	};
	
	class TTT_Vest_Lite_Blue_Us_Woodland : TTT_Vest_Lite_Base {
		scope = 2;
		displayName = "TTT Leichte Weste Blau (US Woodland)";
		hiddenSelections[] = {"camo"};
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\vests\vest_blue_co.paa"};
	};
	
	class TTT_Vest_Heavy_Black_Us_Woodland : TTT_Vest_Heavy_Base {
		scope = 2;
		displayName = "TTT Schwere Weste Schwarz (US Woodland)";
		hiddenSelections[] = {"camo"};
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\vests\vest_black_co.paa"};
	};
	
	class TTT_Vest_Lite_Black_Us_Woodland : TTT_Vest_Lite_Base {
		scope = 2;
		displayName = "TTT Leichte Weste Schwarz (US Woodland)";
		hiddenSelections[] = {"camo"};
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\vests\vest_black_co.paa"};
	};
	
	class TTT_Vest_Heavy_Brown_Us_Woodland : TTT_Vest_Heavy_Base {
		scope = 2;
		displayName = "TTT Schwere Weste Braun (US Woodland)";
		hiddenSelections[] = {"camo"};
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\vests\vest_brown_co.paa"};
	};
	
	class TTT_Vest_Lite_Brown_Us_Woodland : TTT_Vest_Lite_Base {
		scope = 2;
		displayName = "TTT Leichte Weste Braun (US Woodland)";
		hiddenSelections[] = {"camo"};
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\vests\vest_brown_co.paa"};
	};
	
	class TTT_Vest_Heavy_Gold_Us_Woodland : TTT_Vest_Heavy_Base {
		scope = 2;
		displayName = "TTT Schwere Weste Gold (US Woodland)";
		hiddenSelections[] = {"camo"};
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\vests\vest_gold_co.paa"};
	};
	
	class TTT_Vest_Lite_Gold_Us_Woodland : TTT_Vest_Lite_Base {
		scope = 2;
		displayName = "TTT Leichte Weste Gold (US Woodland)";
		hiddenSelections[] = {"camo"};
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\vests\vest_gold_co.paa"};
	};
	
	class TTT_Vest_Heavy_Green_Us_Woodland : TTT_Vest_Heavy_Base {
		scope = 2;
		displayName = "TTT Schwere Weste Gruen (US Woodland)";
		hiddenSelections[] = {"camo"};
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\vests\vest_green_co.paa"};
	};
	
	class TTT_Vest_Lite_Green_Us_Woodland : TTT_Vest_Lite_Base {
		scope = 2;
		displayName = "TTT Leichte Weste Gruen (US Woodland)";
		hiddenSelections[] = {"camo"};
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\vests\vest_green_co.paa"};
	};
	
	class TTT_Vest_Heavy_Grey_Us_Woodland : TTT_Vest_Heavy_Base {
		scope = 2;
		displayName = "TTT Schwere Weste Grau (US Woodland)";
		hiddenSelections[] = {"camo"};
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\vests\vest_grey_co.paa"};
	};
	
	class TTT_Vest_Lite_Grey_Us_Woodland : TTT_Vest_Lite_Base {
		scope = 2;
		displayName = "TTT Leichte Weste Grau (US Woodland)";
		hiddenSelections[] = {"camo"};
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\vests\vest_grey_co.paa"};
	};
	
	class TTT_Vest_Heavy_Red_Us_Woodland : TTT_Vest_Heavy_Base {
		scope = 2;
		displayName = "TTT Schwere Weste Rot (US Woodland)";
		hiddenSelections[] = {"camo"};
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\vests\vest_red_co.paa"};
	};
	
	class TTT_Vest_Lite_Red_Us_Woodland : TTT_Vest_Lite_Base {
		scope = 2;
		displayName = "TTT Leichte Weste Rot (US Woodland)";
		hiddenSelections[] = {"camo"};
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\vests\vest_red_co.paa"};
	};
	
	class TTT_Vest_Heavy_Purple_Us_Woodland : TTT_Vest_Heavy_Base {
		scope = 2;
		displayName = "TTT Schwere Weste Violett (US Woodland)";
		hiddenSelections[] = {"camo"};
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\vests\vest_purple_co.paa"};
	};
	
	class TTT_Vest_Lite_Purple_Us_Woodland : TTT_Vest_Lite_Base {
		scope = 2;
		displayName = "TTT Leichte Weste Violett (US Woodland)";
		hiddenSelections[] = {"camo"};
		hiddenSelectionsTextures[] = {"\ttt_a3\ttt_units_us\woodland\vests\vest_purple_co.paa"};
	};
	
	class TTT_Vest_Crew_Us_Woodland : TTT_Vest_Crew_Base {
		scope = 2;
		displayName = "TTT Crew Weste (US Woodland)";
	};