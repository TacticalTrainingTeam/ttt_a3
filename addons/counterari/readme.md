...
`fnc_counterFire.sqf` und `fnc_positionFire.sqf` behalten ihre öffentliche Parameter-Signatur bei (weiterhin direkt per manuellem `addEventHandler` bzw. Trigger-`onActivation` nutzbar, siehe `docs/addons/counterari.md`), nur die interne Umsetzung wurde überarbeitet. Beide teilen sich seit dem letzten Umbau den eigentlichen Feuermissions-Ablauf (Munition/Besatzung prüfen, Geschütze zufällig verzögert feuern lassen, auf alle Schüsse warten, Munition zurücksetzen, `QGVAR(busy)` wieder freigeben) über `fnc_executeFireMission.sqf` — vorher war dieser Block in beiden Dateien nahezu identisch dupliziert. `fnc_counterFire.sqf` ruft ihn mit der zum Feuerzeitpunkt aktuellen Position des eigenen Geschützes auf (Verzögerung durch `_counterTime`), `fnc_positionFire.sqf` mit der sofort ausgewerteten Position des Ziel-Triggers.
